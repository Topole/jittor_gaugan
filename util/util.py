...
def color_histogram(img):
    ...
    for i in range(img.shape[0]):
        ...
        b_list.append(b_hist)
        g_list.append(g_hist)
        r_list.append(r_hist)
        # print('histlen ',len(b_hist))
        # print('histlen ',b_hist.shape)
    b_list = jt.Var(b_list).view(img.shape[0],256)
    g_list = jt.Var(g_list).view(img.shape[0],256)
    r_list = jt.Var(r_list).view(img.shape[0],256)
    return b_list,g_list,r_list