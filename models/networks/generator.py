...
class SPADEGenerator(BaseNetwork):
    ...
    def __init__(self, opt):
        ...
        self.head_0 = SPADEResnetBlock(16 * nf * 4, 16 * nf, opt)
    ...
    def execute(self, input, z=None, b=None, g=None, r=None):
        seg = input
        ...
        if self.opt.use_vae:
            ...
            b = self.fc(b)
            b = x.view(-1, 16 * self.opt.ngf, self.sh, self.sw)
            g = self.fc(g)
            g = x.view(-1, 16 * self.opt.ngf, self.sh, self.sw)
            r = self.fc(r)
            r = x.view(-1, 16 * self.opt.ngf, self.sh, self.sw)

            x = jt.concat([x,b,g,r],dim=1)
            # print('zbgrxbrg ',z.shape,x.shape,b.shape,g.shape,r.shape)
